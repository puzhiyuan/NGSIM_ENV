...
class KinematicObservation(ObservationType):
    ...
    def observe(self):
        ...
        if close_vehicles:
            ...
            df = pandas.concat([df, pandas.DataFrame.from_records([v.to_dict(origin, observe_intentions=self.observe_intentions)
                                                                  for v in close_vehicles])[self.features]], ignore_index=True)
        # Normalize
        if self.normalize:
        ...
        if df.shape[0] < self.vehicles_count:
            ...
            df = pandas.concat([df, pandas.DataFrame(data=rows, columns=self.features)], ignore_index=True)
